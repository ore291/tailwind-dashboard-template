...
const EditCategory = () => {
  ...
  const { data: slug_category, isLoading : gettingCategory } = useGetCategoryBySlugQuery(slug);
  ...
  const [
    editCategory, // This is the mutation trigger
    { isLoading: isEditing, isSuccess }, // This is the destructured mutation result
  ] = useEditCategoryMutation();
  ...
  const onSubmit = async (data) => {
    ...
    const body = {
        ...
        "parent" : category.id || slug_category.parent
    }

    await editCategory({id :slug_category.id, body });
  };
  return (
    <Layout>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="relative z-0 w-full mb-6 group my-4">
              <input
                ...
                placeholder=" "
                defaultValue={slug_category.name}
                {...register("name")}
              />
              <label
            ...
            <div className="relative z-0 w-full mb-6 group">
              ...
              <textarea
                ...
                defaultValue={slug_category.description}
              ...
              <div className="relative z-0 w-full mb-6 group">
                ...
                >
                  {categories && (categories.map((category) => (
                    <option selected={slug_category.parent === category.id} value={JSON.stringify(category)} key={category.id}>
                      {category.name}
                    </option>
                  )))}
                </select>
