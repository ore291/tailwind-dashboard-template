...
const Rentals = () => {
  ...
  const formatRowData = (rawData) =>
    rawData.map((info) => ({
      id: info.id,
      // title: info.title,
      lender: info.lender.first_name + " " + info.lender.last_name,
      lendee: info.lendee.first_name + " " + info.lendee.last_name,
      item: info.item.title,
      cost : formatCurrency(info.cost),
      paid : info.paid ? "Paid" : "Pending",
      method : info.payment_method,
      duration : `${info.duration} days`,
      start_date : formatDate(info.from_date),
      end_date : formatDate(info.to_date),
      status : info.rental_status
  ...
  const { data, isLoading, isFetching } = useGetRentalsPagQuery({
    id: query || 0,
    page: currentPage,
    size: 20,
  });
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ cell: { value } }) => (
          <React.Fragment>
            <Badge color={check_status(value)}>
              {value}
            </Badge>
          </React.Fragment>
        ),
        ...
        <form className="flex items-center my-5">
            ...
            <div className="relative w-full md:w-[50%]">
              ...
              <input
                ...
                placeholder="Search rental by id"
              />
            </div>
            {/* <button
              type="submit"
